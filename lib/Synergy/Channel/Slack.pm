...
# -- michael, 2019-02-05
async sub send_ephemeral_message ($self, $channel, $user, $text) {
  $text =~ s/&/&amp;/g;
  $text =~ s/</&lt;/g;
  $text =~ s/>/&gt;/g;

  my $http_res = await $self->slack->api_call('chat.postEphemeral', {
    text => $text,
    channel => $channel,
    user => $user,
    as_user => \1,
  ...
  return $JSON->decode($http_res->decoded_content);
}
